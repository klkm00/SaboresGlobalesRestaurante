...
public class DeliveryService {
    ...
    public Optional<DeliveryResponseDTO> actualizar(Long id, DeliveryRequestDTO dto){
        return deliveryRepository.findById(id)
        .map(existing -> {
            existing.setRepartidor(dto.getRepartidor());
            existing.setTarifa(dto.getTarifa());
            existing.setGps(dto.getGps());
            existing.setEstado(dto.getEstado());
            return mapToDTO(deliveryRepository.save(existing));
        });
    }
}